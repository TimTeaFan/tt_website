...
In this section we will first look at how to use `data.table::order()` on the seven examples from above. The aim is to stay close to base R, but account for 'data.table's syntax specific features.
...
Let's take a look at example 3., ordering by a simple expression with `ifelse()`.
```{r}
# 3. Ordering by a simple expression
setorder(
...
As default `dplyr::arrange()` orders columns in ascending order. To reverse this, we can wrap column names in `dplyr::desc()`. `NA` are always sorted last, and there is not argument to change this behavior. Finally, `arrange()` is one of the few one-table verbs that ignores groupings of a `data.frame`, but this behavior can be changed (see info box below).
